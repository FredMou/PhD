...
exp <- exp[ , c('PCBP1', 'PABPN1','PTPRF', 'DANCR', 'MYC')] #exp[ , c('HNRNPL', 'ELAVL1', 'PCBP1', 'PCBP2', 'PABPN1', 'PTPRF', 'MRPL24', 'DANCR', 'MYC', 'TRPM4')]# these 10 genes was used for calculating log2FC for the 10 gene validation under Lorna's suggestion
View(exp)
dim(exp)
...
# 
# 
# clinical <- read.table("/Users/zhuofanmou/OneDrive - University of Exeter/PhD work/my project/my papers/first paper/DFS.PFS/PFS_edgeR/split_kym/final_v8/df_clinic.genemarkers.COX_final.txt", header = T, sep = "\t") 
# View(clinical)
...
lnames_train
data_combined <- df_clinic.genemarkers.COX_train
View(data_combined)
...
# box plot Gleason
pdf(file="Box_Gleason_5_train.pdf",height=6,width=10)
p2_gleasonComp_train <- ggboxplot(plot.info.Gleason, 
                            x = "Gene_symbol", y = "Expression",
                            color = "black", fill = "Group",palette = "lancet",
                            xlab = "", ylab = "Gene expression",
                            main = "Gene expression comparison training cohort (Gleason score: <8 vs. >=8)")+  #Gleason score <8 (293) and ??8 (206)
...
pdf(file="Box_Tstage_5_train.pdf",height=6,width=10)
p3_TComp_train <- ggboxplot(plot.info.T, 
                      x = "Gene_symbol", y = "Expression",
                      color = "black", fill = "Group",palette = "lancet",
                      xlab = "", ylab = "Gene expression",
                      main = "Gene expression comparison training cohort (tumour stage: T2 vs. T3-T4)")+ # tumour stage T2 (189)  and T3-T4 (303)
  stat_compare_means(aes(group = Group), label = "p.signif", method = "wilcox.test", hide.ns = F)+
  theme_bw()+
  theme(panel.background = element_blank(),
        panel.grid = element_blank(),
...
# box plot N
pdf(file="Box_Nstage_5_train.pdf",height=6,width=10)
p4_NComp_train <- ggboxplot(plot.info.N, 
                      x = "Gene_symbol", y = "Expression",
                      color = "black", fill = "Group",palette = "lancet",
                      xlab = "", ylab = "Gene expression",
                      main = "Gene expression comparison training cohort (lymph node status: N0 vs. N1)")+ # Lymph node status: N0(347) vs. N1(79)
  stat_compare_means(aes(group = Group), label = "p.signif", method = "wilcox.test", hide.ns = F)+
  theme_bw()+
  theme(panel.background = element_blank(),
        panel.grid = element_blank(),
...
# box plot Age
pdf(file="Box_age_5_train.pdf",height=6,width=10)
p5_ageComp_train <- ggboxplot(plot.info.age, 
                        x = "Gene_symbol", y = "Expression",
                        color = "black", fill = "Group",palette = "lancet",
                        xlab = "", ylab = "Gene expression",
                        main = "Gene expression comparison training cohort (age: <60 vs. >=60)")+ # age: <60 (203) vs. ??60 (296)
...
# Riskscore group boxplot
library(tidyr)
data_combined_RS <- data_combined %>% drop_na(risk)
dim(data_combined_RS)
View(data_combined_RS)

plot.info.RS <- NULL
for (i in 1:length(geneID)) {
  idx.sub.RS <- which(colnames(data_combined_RS) == geneID[i])
  sub.RS <- data.frame(
    PATIENT_ID = rownames(data_combined_RS),
    Gene_symbol = geneID[i],
    Group = data_combined_RS$risk,
    Expression = data_combined_RS[, idx.sub.RS]
  )
  plot.info.RS <- rbind(plot.info.RS, sub.RS)
}


# box plot RS

pdf(file="Box_riskScore_5_train.pdf",height=6,width=10)
p6_RSComp_train <- ggboxplot(plot.info.RS, 
                            x = "Gene_symbol", y = "Expression",
                            color = "black", fill = "Group",palette = "lancet",
                            xlab = "", ylab = "Gene expression",
                            main = "Gene expression comparison training cohort (risk: high vs. low)")+ # tumour stage T2 (189)  and T3-T4 (303)
  stat_compare_means(aes(group = Group), label = "p.signif", method = "wilcox.test", hide.ns = F)+
  theme_bw()+
  theme(panel.background = element_blank(),
        panel.grid = element_blank(),
...
data_combined <- df_clinic.genemarkers.COX_test
View(data_combined)
...
# box plot Gleason
pdf(file="Box_Gleason_5_test.pdf",height=6,width=10)
p2_gleasonComp_test <- ggboxplot(plot.info.Gleason, 
                                  x = "Gene_symbol", y = "Expression",
                                  color = "black", fill = "Group",palette = "lancet",
                                  xlab = "", ylab = "Gene expression",
                                  main = "Gene expression comparison testing cohort (Gleason score: <8 vs. >=8)")+  #Gleason score <8 (293) and ??8 (206)
...
pdf(file="Box_Tstage_5_test.pdf",height=6,width=10)
p3_TComp_test <- ggboxplot(plot.info.T, 
                            x = "Gene_symbol", y = "Expression",
                            color = "black", fill = "Group",palette = "lancet",
                            xlab = "", ylab = "Gene expression",
                            main = "Gene expression comparison testing cohort (tumour stage: T2 vs. T3-T4)")+ # tumour stage T2 (189)  and T3-T4 (303)
  stat_compare_means(aes(group = Group), label = "p.signif", method = "wilcox.test", hide.ns = F)+
  theme_bw()+
  theme(panel.background = element_blank(),
        panel.grid = element_blank(),
...
# box plot N
pdf(file="Box_Nstage_5_test.pdf",height=6,width=10)
p4_NComp_test <- ggboxplot(plot.info.N, 
                            x = "Gene_symbol", y = "Expression",
                            color = "black", fill = "Group",palette = "lancet",
                            xlab = "", ylab = "Gene expression",
                            main = "Gene expression comparison testing cohort (lymph node status: N0 vs. N1)")+ # Lymph node status: N0(347) vs. N1(79)
  stat_compare_means(aes(group = Group), label = "p.signif", method = "wilcox.test", hide.ns = F)+
  theme_bw()+
  theme(panel.background = element_blank(),
        panel.grid = element_blank(),
...
# box plot Age
pdf(file="Box_age_5_test.pdf",height=6,width=10)
p5_ageComp_test <- ggboxplot(plot.info.age, 
                              x = "Gene_symbol", y = "Expression",
                              color = "black", fill = "Group",palette = "lancet",
                              xlab = "", ylab = "Gene expression",
                              main = "Gene expression comparison testing cohort (age: <60 vs. >=60)")+ # age: <60 (203) vs. ??60 (296)
...
pdf(file="Box_riskScore_5_test.pdf",height=6,width=10)
p6_RSComp_test <- ggboxplot(plot.info.RS, 
                             x = "Gene_symbol", y = "Expression",
                             color = "black", fill = "Group",palette = "lancet",
                             xlab = "", ylab = "Gene expression",
                             main = "Gene expression comparison testing cohort (risk: high vs. low)")+ # tumour stage T2 (189)  and T3-T4 (303)
  stat_compare_means(aes(group = Group), label = "p.signif", method = "wilcox.test", hide.ns = F)+
  theme_bw()+
  theme(panel.background = element_blank(),
        panel.grid = element_blank(),
